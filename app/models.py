...
class User(UserMixin, db.Model):
    __tablename__ = 'users'  # Explicitly defining table name
...
class Job(db.Model):
    ...
    salary = db.Column(db.String(100), nullable=True)  # Salary or salary range
...
class Application(db.Model):
    ...
    resume = db.Column(db.String(255), nullable=True)  #Path to the resume file
    cover_letter = db.Column(db.Text, nullable=True)  #Cover letter content
